...
ДОДАТОК ДО ІНСТРУКЦІЇ: ВЗАЄМОДІЯ ТА СИСТЕМА ІНСТРУМЕНТІВ (AGENTIC TOOLS)
1. Моделі взаємодії з користувачем (User Interaction)
Interactive Chat (UI): У WPF-інтерфейсі AsmodayCat.UI реалізується повноцінний чат (з підтримкою Markdown та підсвічування синтаксису коду). Користувач може ставити питання, просити написати код або створити картинку.

Project Context (Робота з проєктами): Користувач може "закинути" в чат шлях до папки з проєктом (наприклад, Solution у Visual Studio). Агент автоматично індексує структуру файлів і може виконувати масовий рефакторинг, створювати нові класи або писати тести.

System Tray / Global Hotkey: Можливість викликати міні-вікно AsmodayCat поверх усіх вікон (як Spotlight) для швидких запитів, перекладу або генерації коду без відкриття головного вікна UI.

2. Розширені Функціональні Вимоги (FR)
FR7: Система інструментів агента (Agent Tooling / Function Calling)
Агент повинен підтримувати реєстрацію зовнішніх інструментів, які LLM може викликати за потреби:

FR7.1 (Web Search & Browsing): Інструмент доступу до інтернету. Агент формує пошуковий запит, інструмент виконує пошук, парсить вебсторінку (знімає зайвий HTML, залишає текст) і повертає контент у контекст моделі. Увесь інтернет-трафік агента проходить через маршрутизацію та фільтри BlackCat.

FR7.2 (Code & Shell Execution): Інструмент виконання команд у консолі. Агент може виконувати git команди, збирати проєкти (наприклад, dotnet build), створювати файли та директорії.

FR7.3 (Image Generation): Інструмент генерації зображень. Якщо запит стосується графіки, агент формує промпт для дифузійної моделі (інтеграція з локальним Stable Diffusion API / ComfyUI або зовнішнім API) та зберігає результат у вказану папку.

FR7.4 (Project Manager): Інструмент аналізу кодової бази. Агент може прочитати дерево директорій, знайти потрібні файли, проаналізувати їх і внести зміни.

3. ДОДАТОК ДО ДЕКОМПОЗИЦІЇ (ТАCКИ ДЛЯ CLAUDE)
@Claude: Додай наступну фазу до свого плану розробки. Вона стосується розширення можливостей автономного агента та UI.

Phase 7: Agent Tools & Function Calling (Розширення AsmodayCat.Agent)
Tool Registry (/Tools):

Створити базовий інтерфейс IAgentTool з методами GetName(), GetDescription() (для передачі в LLM як доступної функції) та ExecuteAsync(string jsonArguments).

Реалізувати ToolManager, який реєструє всі доступні інструменти та обробляє запит від LLM на їх виклик.

Internet Access Tool (/Tools/Web):

Реалізувати WebSearchTool та WebScraperTool.

Важливо: Налаштувати мережевий клієнт (HttpClient) так, щоб він звертався до інтернету через проксі/правила BlackCatBridge (використовувати інтерфейси з Phase 4).

Project & Shell Tool (/Tools/System):

Реалізувати ShellExecutionTool для виконання команд cmd.exe / powershell. Додати жорсткі ліміти та валідацію команд задля безпеки.

Реалізувати FileSystemTool (CreateFile, ReadFile, ReadDirectoryStructure).

Chat Interface (AsmodayCat.UI):

Реалізувати ChatViewModel та ChatView у WPF.

Додати підтримку відображення Markdown (використати готові NuGet-пакети для WPF Markdown, наприклад, Markdig.Wpf).

Додати індикатор "Агент думає..." та "Агент використовує інструмент: [Назва інструменту]...".

Global Hotkey (UI):

Додати логіку перехоплення глобальної гарячої клавіші (наприклад, Ctrl+Space або Alt+C) для швидкого виклику міні-вікна (Overlay) поверх інших вікон.

ДОДАТОК ДО ІНСТРУКЦІЇ: МАТРИЦЯ МОДЕЛЕЙ ТА АВТОНОМНЕ ЗАВАНТАЖЕННЯДля забезпечення максимальної автономності, AsmodayCat повинен підтримувати роботу з пулом локальних моделей, кожна з яких оптимізована під конкретні задачі, та вміти самостійно ними керувати.1. Автономне управління моделями (Auto-Pull System)FR8: Механізм автономного забезпечення моделямиFR8.1: При отриманні задачі або підключенні нового інструменту, AsmodayCat.Core зобов'язаний перевірити локальний реєстр (наприклад, через ендпоінт /api/tags для Ollama). Якщо необхідна модель відсутня, оркестратор автоматично ініціює її завантаження (ендпоінт /api/pull).FR8.2: Процес завантаження повинен супроводжуватися відправкою поточного прогресу через IPC до AsmodayCat.UI для відображення індикатора скачування користувачу.FR8.3: Усі мережеві запити на скачування ваг моделей повинні строго маршрутизуватися через захищені канали BlackCat.2. Матриця цільових моделей (LLM Pool Matrix)Система повинна мати вбудовані конфігурації для автоматичного розгортання наступного пулу інструментів:Цільова задачаРекомендована локальна модельОпис та інтеграція в екосистемуОсновний Агент (General / Function Calling)mistral:7b (або llama3:8b)Ядро оркестрації. Володіє високою здатністю до логічного висновку та вміє викликати інструменти (Tools). Вирішує, коли звернутися до інтернету, а коли працювати з локальними файлами.Розробка та Архітектура (Coding)qwen2.5-coder:7bСпеціалізована модель для кодової бази. Оптимізована для глибокого аналізу проєктів, рефакторингу, написання тестів та роботи зі стеком (C#, .NET, React, Node.js, Python).Швидкий моніторинг (Fast Analytics)phi3:3.8b (або gemma2:2b)Легковагова модель для миттєвої реакції. Запускається агентом-спостерігачем для швидкого парсингу нових файлів або логів у локальних "пісочницях" без перевантаження VRAM.Аналіз зображень (Computer Vision)llava:7bМультимодальна модель для розпізнавання тексту зі скріншотів, аналізу діаграм та візуальних даних, які потрапляють у робочі директорії агента.Генерація зображень (Image Generation)Stable Diffusion (API Bridge)Інтеграція з локальним SD (наприклад, ComfyUI або WebUI API). Агент формує деталізовані промпти (включаючи задану стилістику, як-от monochrome або dark fantasy) і керує збереженням артефактів.3. Динамічне перемикання та Гібридний режим (Model Swapping & Fallback)FR9: Оптимізація ресурсів та делегуванняFR9.1 (Контекстне перемикання): Система повинна автоматично вивантажувати неактивні моделі з VRAM та завантажувати потрібні під поточний контекст (наприклад, перемикання з mistral на qwen2.5-coder при отриманні команди на генерацію коду).FR9.2 (Cloud Fallback): Якщо локальне апаратне забезпечення перевантажене або задача вимагає надвисокої когнітивної складності, AsmodayCat може передати промпт зовнішнім API (наприклад, Claude Pro). Цей трафік обов'язково фільтрується через правила BlackCat.

| Цільова задача | Рекомендована локальна модель | Опис та інтеграція в екосистему |
| :--- | :--- | :--- |
| **Основний Агент (General / Function Calling)** | `mistral:7b` (або `llama3:8b`) | Ядро оркестрації. Володіє високою здатністю до логічного висновку та вміє викликати інструменти (Tools). Вирішує, коли звернутися до інтернету, а коли працювати з локальними файлами. |
| **Розробка та Архітектура (Coding)** | `qwen2.5-coder:7b` | Спеціалізована модель для кодової бази. Оптимізована для глибокого аналізу проєктів, рефакторингу, написання тестів та роботи зі стеком (C#, .NET, React, Node.js, Python). |
| **Швидкий моніторинг (Fast Analytics)** | `phi3:3.8b` (або `gemma2:2b`) | Легковагова модель для миттєвої реакції. Запускається агентом-спостерігачем для швидкого парсингу нових файлів або логів у локальних "пісочницях" без перевантаження VRAM. |
| **Аналіз зображень (Computer Vision)** | `llava:7b` | Мультимодальна модель для розпізнавання тексту зі скріншотів, аналізу діаграм та візуальних даних, які потрапляють у робочі директорії агента. |
| **Генерація зображень (Image Generation)** | `Stable Diffusion` (API Bridge) | Інтеграція з локальним SD (наприклад, ComfyUI або WebUI API). Агент формує деталізовані промпти (включаючи задану стилістику) і керує збереженням артефактів. |