...
namespace DocControlService
{
    ...
    public partial class DocControlWindowsService : ServiceBase
    {
        ...
        private async Task<ServiceResponse> ProcessCommand(string requestJson)
        {
            try
            {
                ...
                switch (command.Type)
                {
                    ...
                    case CommandType.UpdateDeviceAccess:
                        return HandleUpdateDeviceAccess(command.Data);

                    case CommandType.GetDirectoryAccessList:
                        return HandleGetDirectoryAccessList(command.Data);

                    case CommandType.GrantDirectoryAccess:
                        return HandleGrantDirectoryAccess(command.Data);

                    case CommandType.RevokeDirectoryAccess:
                        return HandleRevokeDirectoryAccess(command.Data);
        ...
        private ServiceResponse HandleGetDevices()
        {
            ...
            {
                Log($"Error getting devices: {ex.Message}", EventLogEntryType.Error);
                return new ServiceResponse
                {
                    Success = false,
                    Message = ex.Message
        ...
        private ServiceResponse HandleUpdateDeviceAccess(string data)
        {
            try
            {
                var request = JsonSerializer.Deserialize<UpdateDeviceAccessRequest>(data);
                bool success = _deviceRepo.UpdateDevice(request.DeviceId, null, request.HasAccess);

                return new ServiceResponse
                {
                    Success = success,
                    Message = success ? "Access updated" : "Device not found"
                };
            }
            catch (Exception ex)
            {
                Log($"Error updating device access: {ex.Message}", EventLogEntryType.Error);
                return new ServiceResponse
                {
                    Success = false,
                    Message = ex.Message
        ...
        private ServiceResponse HandleGetDirectoryAccessList(string data)
        {
            try
            {
                var request = JsonSerializer.Deserialize<DirectoryAccessListRequest>(data);
                var accessList = _networkAccessRepo.GetAccessByDirectory(request.DirectoryId);
            ...
            {
                Log($"Error getting directory access list: {ex.Message}", EventLogEntryType.Error);
                return new ServiceResponse
                {
                    Success = false,
                    Message = ex.Message
        ...
        private ServiceResponse HandleGrantDirectoryAccess(string data)
        {
            try
            {
                var request = JsonSerializer.Deserialize<AccessRequest>(data);
                int id = _networkAccessRepo.GrantAccess(request.DirectoryId, request.DeviceId);
            ...
            {
                Log($"Error granting directory access: {ex.Message}", EventLogEntryType.Error);
                return new ServiceResponse
                {
                    Success = false,
                    Message = ex.Message
        ...
        private ServiceResponse HandleRevokeDirectoryAccess(string data)
        {
            try
            {
                var request = JsonSerializer.Deserialize<AccessRequest>(data);
                bool success = _networkAccessRepo.RevokeAccess(request.DirectoryId, request.DeviceId);

                return new ServiceResponse
                {
                    Success = success,
                    Message = success ? "Access revoked" : "Access entry not found"
                };
            }
            catch (Exception ex)
            {
                Log($"Error revoking directory access: {ex.Message}", EventLogEntryType.Error);
                return new ServiceResponse
                {
                    Success = false,
                    Message = ex.Message
