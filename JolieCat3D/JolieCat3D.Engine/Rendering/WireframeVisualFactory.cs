...
namespace JolieCat3D.Engine.Rendering
{
    ...
    /// meshed node's own edges (via <see cref="Core.Geometry.Mesh.GetEdges"/>,
    /// transformed by that node's <see cref="CoreNode.GetWorldTransform"/>), all as one
    /// <see cref="LinesVisual3D"/>. The same building block
    /// <c>Editing.ComponentMarkerVisualFactory</c>'s own edge markers already use, just
    /// walking the WHOLE scene tree instead of one Edit Mode target.
    /// </summary>
    public static class WireframeVisualFactory
    {
        ...
        private static void CollectEdges(CoreNode node, Point3DCollection points)
        {
            ...
            {
                var worldA = Vector3.Transform(mesh.Vertices[a].Position, world);
                var worldB = Vector3.Transform(mesh.Vertices[b].Position, world);
